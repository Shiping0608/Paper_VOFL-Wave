function test()
% -------------------------------------------------------------------------
% test()  Driver function for running CN scheme temporal convergence tests.
%
%   This script sets up a simple test problem and calls the CN solver 
...
%
%   Subroutines:
%       CN(xb, xe, alpha, T0, T1, N, x, s, sp, dt, lambda)
...
M = 30; % number of expansion terms
% -------------------------------------------------------------------------
% Run simulation for each dt
for dt = dts
    h = (xe-xb)/N;
    x = [xb:h:xe-h]';            % grid points
    s = 1 + 0.3*sin(0.125*pi*x); % spatial dependent s(x)
    sp = 0.5*(max(s)+min(s));
    CN(xb, xe, alpha, T0, T1, N, x, s, sp, dt, lambda, M-1);
end
error_ref()
