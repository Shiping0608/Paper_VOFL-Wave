function splitting(xb, xe, N, x, T0, T1, dt, alpha, phi, psi, sp, s, M)
% -------------------------------------------------------------------------
...
solution = zeros(N,double(T1/0.1));
solution(:,1) = u0;
% figure
for n=1:NT
    % ---------------------------------------------------------------------
    ...
    tn = n*dt;
    if mod(tn,0.1) == 0
        solution(:,round(tn/0.1)+1) = real(ifft(u3_hat));
    end
end
save("data_sin_N"+num2str(N)+ "_t"+num2str(tn)+".mat",'x', ...
      'T0','T1','dt','tn','solution');