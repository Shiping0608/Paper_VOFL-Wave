function [t1, t2, t3] = fractionalWave_splitting(xb, xe, N, T0, T1, dt, alpha)
...
DM2_s = real((exp(1i*x.'*mu).*(abs(mu).^(2*s)))*exp(-1i*mu.'*x)/N);
...
for n=1:NT
    ...
    %u2 = u1;
    DM2_sp_u1 = ifft(abs(mu').^(2*sp).*temp1);
    v2 = v1 + c2*dt*(alpha*(-DM2_sp_u1+DM2_s*u1)+lambda*u1.^3);
